...
//
// `tolerateFailures` is for an endpoint that may not exist at all: one term
// failing should not lose the results the other terms found, but every term
// failing still has to raise, or a dead endpoint would read as "no matches".
async function unionSearches(terms, run, identify, { tolerateFailures = false } = {}) {
  const settled = await Promise.all(terms.map(t => run(t).then(
    (value) => ({ ok: true, value }),
    (error) => ({ ok: false, error }))))

  const failures = settled.filter(r => !r.ok)
  if (failures.length === terms.length) throw failures[0].error
  if (!tolerateFailures && failures.length) throw failures[0].error
...
async function searchPolymarket(terms) {
  let events = await unionSearches(terms, async (term) => {
  ...
  }, e => e.slug, { tolerateFailures: true })

  if (!events.length) {
    // public-search is not part of the documented gamma surface. When it
    // answers nothing — or is not there at all — fall back to the documented
    // listing and filter it here on the same terms.
    const r = await fetchJson(
    ...
    events = all.filter(e => match.searchTokens(e.title || "").some(t => wanted.has(t)))
  }
...
async function findCrossPlatform(source, deps = {}) {
  ...
  const settled = await Promise.all(targets.map(async (platform) => {
    ...
    try {
      const { value: listings } = await cached(key, SEARCH_CACHE_MS, () => searchers[platform](terms))

      let candidates = match.rankCandidates(source, listings, 6)
      if (candidates.length) {
        if (platform === "kalshi" && signedGet) await enrichKalshi(candidates, signedGet)
        if (platform === "polymarket" && !deps.searchPolymarket) await enrichPolymarket(candidates)
