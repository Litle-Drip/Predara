...
const KALSHI_F1_SERIES = "KXF1RACE"
...
async function searchPolymarketGlobal(terms) {
...
// Polymarket US has no public JSON catalog, but its F1 series page is publicly
// server-rendered and carries stable event-card records. Read those records
// instead of guessing a nonexistent API hostname.
function polymarketUsF1Candidates(html) {
  const candidates = []
  const seen = new Set()
  const card = /\\"cell\\":\{\\"id\\":\\"[^"]+\\",\\"slug\\":\\"(f1-[^"]+)\\",\\"href\\":\\"([^"]+)\\",\\"title\\":\\"([^"]+)\\"/g
  let found
  while ((found = card.exec(String(html || "")))) {
    const [, ref, href, rawTitle] = found
    ...
    candidates.push({
      ...
      subtitle: "",
      date: match.extractDate(ref),
      outcomes: [],
      url: `https://polymarket.us${href}`,
      ref,
    })
  }
  return candidates
...
async function searchPolymarketUsF1(terms) {
  const termSet = new Set(terms)
  const isF1Search = termSet.has("f1") || termSet.has("formula") || termSet.has("gp") ||
    (termSet.has("grand") && termSet.has("prix"))
  if (!isF1Search) return []
  const response = await fetchJson("https://polymarket.us/sports/f1")
  if (response.status !== 200) throw new Error(`Polymarket US search returned ${response.status}`)
  const wanted = new Set(terms)
  return polymarketUsF1Candidates(response.body)
    .filter(c => match.searchTokens(c.title).some(t => wanted.has(t)))
}

async function searchPolymarket(terms) {
  const [global, us] = await Promise.allSettled([
    searchPolymarketGlobal(terms),
    searchPolymarketUsF1(terms),
  ])
  const candidates = [
    ...(global.status === "fulfilled" ? global.value : []),
    ...(us.status === "fulfilled" ? us.value : []),
  ]
  if (!candidates.length && global.status === "rejected") throw global.reason

  const unique = new Map()
  for (const c of candidates) {
    const identity = `${match.normalizeTitle(c.title)}:${c.date}`
...
async function searchKalshi(terms, signedGet) {
  ...
  let indexError = null
  try {
    index = await kalshiIndex(signedGet)
  } catch (err) {
    indexError = err
  }
  const combined = [...index]
  const termSet = new Set(terms)
  const isGrandPrixSearch = termSet.has("gp") ||
    (termSet.has("grand") && termSet.has("prix"))
  if (isGrandPrixSearch) {
    let series = []
    try {
      const r = await signedGet(
        `/trade-api/v2/events?status=open&limit=${KALSHI_PAGE_SIZE}` +
        `&series_ticker=${KALSHI_F1_SERIES}&with_nested_markets=false`)
      if (r && r.status === 200) series = jsonList(parseJson(r.body), "events")
    } catch { /* the broad index remains usable */ }
    if (series) {
      const seen = new Set(combined.map(e => e.event_ticker))
      for (const e of series) if (!seen.has(e.event_ticker)) combined.push(e)
    }
  }
  if (!combined.length && indexError) throw indexError
  const wanted = new Set(terms)
  return combined
...
async function enrichPolymarket(candidates) {
  await Promise.all(candidates.slice(0, ENRICH_TOP_N).map(async (c) => {
    ...
    if ((c.outcomes || []).length || c.venue === "us") return
    try {
      const r = await fetchJson(
...
async function enrichKalshi(candidates, signedGet) {
  await Promise.all(candidates.slice(0, ENRICH_TOP_N).map(async (c) => {
    ...
    try {
      ...
      c.date = match.extractDate(
        first.occurrence_datetime,
        first.expected_expiration_time,
        event.strike_date,
        first.close_time,
      ) || c.date
    } catch { /* keep the unenriched candidate */ }
...
function topByCloseness(source, listings, limit) {
  return (listings || [])
    .map(l => ({ listing: l, near: enrichmentCloseness(source, l) }))
...
function enrichmentCloseness(source, listing) {
  const sourceScope = match.eventScope(`${source.title || ""} ${source.subtitle || ""}`)
  const listingScope = match.eventScope(`${listing.title || ""} ${listing.subtitle || ""}`)
  if (sourceScope && listingScope && sourceScope !== listingScope) return 0
  ...
  const outcomeOverlap = match.overlapRatio(
    match.outcomeKeys(source.outcomes),
    match.outcomeKeys(listing.outcomes)) || 0
  const gap = match.daysApart(source.date, listing.date)
  const dateNear = gap == null ? 0 : gap === 0 ? 1 : gap <= 1 ? 0.7 : gap <= 3 ? 0.55 : 0
  return titleOverlap * 0.45 + outcomeOverlap * 0.25 + dateNear * 0.3
}
...
module.exports = {
  ...
  searchGemini,
  searchPolymarket,
  polymarketUsF1Candidates,
  searchPolymarketUsF1,
  searchKalshi,
  kalshiIndex,
  enrichKalshi,
  PLATFORMS,
