...
function makeSignedGet() {
  ...
  return async (apiPath) => {
    ...
    for (const hostname of KALSHI_HOSTS) {
      // A DNS failure or a timeout on the first host has to fall through to the
      // next one too, not reject the whole lookup — otherwise one unreachable
      // host takes down every Kalshi read even while the other host is serving.
      try {
