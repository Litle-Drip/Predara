...
//
// In-flight calls are shared, not just completed ones. Without this the "at
// most once" above was untrue for the case that matters: a burst of readers
// arriving on a cold key each ran the producer, which for the signed Kalshi
// routes is one upstream request per reader against a rate limit Predara
// shares. The first caller now does the work and the rest await it.
...
async function cached(key, ttlMs, producer) {
  ...
  if (pending) return { value: await pending, fromCache: true }

  // Errors stay uncached: the key is released in `finally`, so the next caller
  // retries rather than inheriting the failure.
  const work = (async () => producer())()
