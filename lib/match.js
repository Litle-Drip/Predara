...
function searchTerms(source, max = 7) {
  ...
  // Alternate spellings go on the end: they are worth asking for, but never at
  // the cost of a word the source venue actually used.
  const aliases = []
  for (const t of out) {
  ...
  return [...out, ...aliases].slice(0, max)
}
