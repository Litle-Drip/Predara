...
// These describe the bet, not the event. marketKind() scores them separately;
// leaving them in title overlap made any two "winner" markets look related.
...
const MARKET_KINDS = [
  ...
  ["constructor", /\bwinning constructor\b|\bconstructor winner\b|\bconstructors? champion\b/],
  ["margin",   /\bmargin\b|\bspread\b|\bhandicap\b/],
  // These patterns are tested against normalizeTitle() output, where every
  // run of non-alphanumerics has already collapsed to a single space — so
  // "Over/Under 45.5" arrives as "over under 45 5" and the slashed spellings
  // could never fire, leaving bare "total" as the only live alternative and a
  // title that says "Over/Under" declaring no kind at all.
  ["total",    /\bover under\b|\bo u\b|\btotal\b/],
  // After `total`: "Total Points" is a totals market, and matching `points`
  // first made it disqualify an identically-shaped "… Total" on the other
  // venue as a different market type.
...
function titleTokens(text) {
  return normalizeTitle(text)
    ...
    .filter(t => t && t.length > 1 && !STOPWORDS.has(t) &&
      !NON_IDENTITY_TOKENS.has(t) && !/^(19|20)\d{2}$/.test(t))
}
...
function eventScope(text) {
  const s = normalizeTitle(text)
  if (/\bchampion\b|\bseason\b|\bdrivers title\b|\bconstructors title\b/.test(s)) return "season"
  if (/\baction of the year\b|\baward\b/.test(s)) return "award"
  if (/\bgp\b|\bgrand prix\b|\brace\b/.test(s)) return "race"
  return null
}
...
function scoreMatch(source, candidate) {
  ...
  if (gap != null) {
    ...
    dateScore = gap === 0 ? 1 : gap <= 1 ? 0.7 : gap <= 3 ? 0.55 : 0
  }

  const srcScope = eventScope(srcTitle)
  const canScope = eventScope(canTitle)
  if (srcScope && canScope && srcScope !== canScope) {
    return {
      score: 0,
      confidence: "none",
      reasons: [],
      disqualified: `different event scope (${srcScope} vs ${canScope})`,
    }
  }

  const bothRaces = srcScope === "race" && canScope === "race"
  if (bothRaces && gap != null && gap > 3) {
    return {
      score: 0,
      confidence: "none",
      reasons: [],
      disqualified: `different race dates (${Math.round(gap)} days apart)`,
    }
  }
  ...
  // The mirror image: every competitor in common and not one word of the title.
  // A whole sport's entrants appear in every market that sport has — the race,
  // the season championship, the award for action of the year — so a full
  // outcome overlap says "same sport", not "same event". Asked for the Spanish
  // Grand Prix, this is what offered "F1 Drivers' Champion" and "F1: Action of
  // the Year", each matching 90-100% of the drivers and sharing no word with
  // the race at all.
  //
  // Two listings of one event agree on the words or they agree on the exact
  // day. Sharing neither, while sharing a field of competitors, is a different
  // market about the same sport.
  // "Agree on the day" means the same day within the tolerance dateScore
  // already grants — one day, to absorb the skew between a UTC close timestamp
  // and a local event date. Demanding an exact match here contradicted that and
  // would have rejected a genuine pair whose venues close a few hours either
  // side of midnight.
  const DATE_AGREES = 0.7   // dateScore for a gap of one day or less
  if (titleScore === 0 && (dateScore == null || dateScore < DATE_AGREES)) {
    return {
      score: 0,
      confidence: "none",
      reasons: [],
      disqualified: dateScore == null
        ? "nothing in the title matches and no date to check"
        : "nothing in the title matches and the dates differ",
    }
  }
  ...
  if (bothRaces && !(outcomeScore > 0)) {
    const raceWords = new Set(["f1", "formula", "gp", "race", "motorsport", "motorsports"])
    const distinctiveOverlap = [...srcTokens].some(t => canTokens.has(t) && !raceWords.has(t))
    if (!distinctiveOverlap) {
      return {
        score: 0,
        confidence: "none",
        reasons: [],
        disqualified: "only generic race words overlap",
      }
    }
  }
  ...
  else if (dateScore != null && dateScore >= 0.55) reasons.push("event dates within three days")
...
function searchTokens(text) {
  return String(text || "")
    ...
    .trim()
    .split(" ")
    // Two-character tokens are noise ("de", "la") with two exceptions: ones
    // carrying a digit are series codes ("f1", "t20"), and ones we know an
    // expansion for are abbreviations a venue may well have used instead of
    // the long form ("gp"). Dropping "gp" meant a listing titled "Madrid GP"
    // could be asked for upstream and still not recognised locally.
    .filter(t => t && !STOPWORDS.has(t) && !NON_IDENTITY_TOKENS.has(t) &&
      !/^(19|20)\d{2}$/.test(t) &&
      (t.length > 2 || /\d/.test(t) || Object.prototype.hasOwnProperty.call(SEARCH_ALIASES, t)))
}

// Both spellings of the things venues abbreviate. searchTokens() deliberately
// does not alias, because a query has to use the venue's own spelling — but we
// do not know which spelling that is. Kalshi writes "Grand Prix"; Polymarket
// may write "Madrid GP", which shares no searchable word with it at all. So
// both forms are asked for, and whichever the venue uses answers.
...
// lists Verstappen.
// Room set aside for alternate spellings, and the floor below which the
// source's own words are never traded away for them.
const MAX_ALIAS_TERMS = 2
const MIN_RAW_TERMS = 3

function searchTerms(source, max = 7) {
  ...
  // Alternate spellings go behind the words the source venue actually used, but
  // they get reserved room rather than whatever is left over. Appending them
  // and then truncating dropped every alias as soon as a title had enough words
  // to fill the cap on its own — silently disabling the abbreviation search on
  // long descriptive titles, which are the ones most likely to need it.
  const aliases = []
  for (const t of out) {
    for (const alias of SEARCH_ALIASES[t] || []) {
      if (!seen.has(alias)) { seen.add(alias); aliases.push(alias) }
    }
  }
  const aliasRoom = Math.min(aliases.length, MAX_ALIAS_TERMS)
  // Never trade away the source's own words entirely: a floor keeps the most
  // distinctive terms even when there are many aliases to fit.
  const rawRoom = Math.max(max - aliasRoom, MIN_RAW_TERMS)
  return [...out.slice(0, rawRoom), ...aliases.slice(0, aliasRoom)].slice(0, max)
}

const api = {
  normalizeTitle, titleTokens, marketKind, eventScope, outcomeKey, outcomeKeys,
