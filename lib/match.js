...
// significant word as well as in full. Team and yes/no outcomes fall out of the
// same rule harmlessly.
...
function outcomeKeys(names) {
  ...
  for (const n of names || []) {
    ...
    if (key && key.length > 1) set.add(key)
  }
  return set
}
...
// The words worth sending to a venue's own search box. Venue names and market
// kinds are the parts a search engine can act on; ordering by length puts the
// distinctive words first because some upstreams cap the query.
function searchTerms(source, max = 4) {
  const tokens = titleTokens([source.title, source.subtitle].filter(Boolean).join(" "))
  const seen = new Set()
  const out = []
...
const api = {
  ...
  rankCandidates, searchTerms, STOPWORDS,
}
