...
async function getDiscoveryFeed({
  limit = 8,
  fetchPolymarket = fetchJson,
  fetchGemini = getGeminiPublic,
} = {}) {
  const safeLimit = Math.min(12, Math.max(1, parseInt(limit, 10) || 8))
  const { value } = await cached(`discovery:${safeLimit}`, DISCOVERY_CACHE_MS, async () => {
    const polymarketUrl =
    ...
    const [polymarketResult, geminiResult] = await Promise.allSettled([
      fetchPolymarket(polymarketUrl).then((data) =>
        polymarketEventsToDiscoverCards(data, safeLimit)),
      fetchGemini("events", { status: "active", limit: String(Math.max(24, safeLimit)) })
        .then(({ status, json }) =>
