...
const API_HOST = "api.gemini.com"
const BASE = "/v1/prediction-markets"
...
const RESOURCES = {
  ...
  strike: (q) => {
    if (!isTicker(q.ticker)) throw new BadRequest("Missing or invalid ticker")
    return `${BASE}/events/${encodeURIComponent(q.ticker)}/strike`
  },
  ...
  // ── Combos (multi-leg contracts; not enabled in production yet) ──
  combos: (q) => {
    if (q.instrumentSymbol) {
      if (!isTicker(q.instrumentSymbol)) throw new BadRequest("Invalid instrumentSymbol")
      return `${BASE}/combos/${encodeURIComponent(q.instrumentSymbol)}`
    }
    const sp = new URLSearchParams()
    if (q.status) {
      if (!isLabel(q.status)) throw new BadRequest("Invalid status")
      sp.set("status", q.status)
    }
    sp.set("limit", String(clampInt(q.limit, 1, 100, 24)))
    sp.set("offset", String(clampInt(q.offset, 0, 10000, 0)))
    return `${BASE}/combos?${sp}`
  },
...
function fetchJson(reqPath) {
  return new Promise((resolve, reject) => {
    const req = https.get(
      { hostname: API_HOST, path: reqPath, headers: { Accept: "application/json" } },
      (apiRes) => {
        let body = ""
