...
function parseHourlyVolume(payload, date) {
  ...
  // Several entries can legitimately share an hour — a feed broken out by
  // category reports each category's slice separately — so hours are summed
  // rather than pushed as separate points. Plotting them separately is what
  // made one day's data look like nine.
  const byHour = new Map()
  let unresolved = 0
  let readCount = 0

  const add = (rawHour, volume, index, total) => {
    readCount += 1
    const hour = normalizeHour(rawHour, date, index, total)
    if (hour === null) { unresolved += 1; return }
    byHour.set(hour, (byHour.get(hour) || 0) + volume)
  }

  if (Array.isArray(container)) {
    const rows = container.filter((e) => e && typeof e === "object" && firstNumeric(e, VOLUME_KEYS) !== null)
    rows.forEach((entry, i) => add(firstDefined(entry, HOUR_KEYS), firstNumeric(entry, VOLUME_KEYS), i, rows.length))
  } else if (container && typeof container === "object") {
    const rows = Object.entries(container).filter(([, v]) => toNumber(v) !== null)
    rows.forEach(([key, value], i) => add(key, toNumber(value), i, rows.length))
  }

  if (!byHour.size) return null

  const buckets = [...byHour.entries()]
    .map(([hour, volume]) => ({ hour, volume: round(volume, 2) }))
  ...
  return {
    ...
    // Surfaced rather than swallowed: entries whose hour could not be read are
    // missing from the chart, and the page has to be able to say so.
    unresolved,
    readCount,
  }
}
