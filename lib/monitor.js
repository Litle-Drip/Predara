...
// Gemini states mutual exclusivity under several spellings — kyle.js has
// checked all four for a while, for the same reason. Returns null when nothing
// states it, which is NOT the same as false: an unstated event falls through to
// the template check rather than being called non-exclusive.
//
// The remaining branches of kyle.js kyleExclusive do not apply to this page:
// its single-contract and `type: "binary"` cases are already excluded here for
// having fewer than two live contracts, and its settled-event
// `resolutionSide` evidence cannot help a monitor that filters settled
// contracts out before it gets this far.
const EXCLUSIVITY_KEYS = ["mutuallyExclusive", "mutually_exclusive", "isMutuallyExclusive", "exclusive"]

function statedExclusivity(event) {
  const e = event || {}
  for (const key of EXCLUSIVITY_KEYS) {
...
function readQuote(contract) {
  ...
  // The contract's own minimum price step. A leg offered AT that step cannot be
  // quoted any cheaper, so its price is bounded by the tick rather than by what
  // anyone thinks the outcome is worth — see the tick-floor note on
  // eventOverround.
  const tick = positivePrice(contract && contract.priceIncrement)
  ...
  return {
    ...
    tick,
    atTickFloor: ask !== null && tick !== null && ask <= tick + 1e-9,
...
//
// So only events stated to be single-winner are eligible, and every leg must be
...
//
// The returned value is decomposed, because on a wide field the headline is
// misleading on its own. Every outcome has a `priceIncrement` — a cent on the
// markets seen so far — and a longshot whose true chance is a tenth of that
// still cannot be offered below it. The US Open men's singles winner market
// carries ~96 players: if ninety of them rest at $0.01, that is $0.90 of "sum
// of asks" contributed by granularity rather than by any margin the venue is
// charging, and the event reports +57% while being priced perfectly sensibly.
// So `tickFloorLegs` and `tickFloorSum` travel with the value and the page
// shows what share of the sum sits on the floor.
function eventOverround(event) {
  ...
  // An explicit flag outranks the template. kyle.js has resolved exclusivity
  // this way for a while because Gemini states it under several spellings, and
  // a stated fact beats an inference from the market's shape. Only a boolean
  // counts: absent means unstated, not false.
  const stated = statedExclusivity(event)
  if (stated === false) {
    return { value: null, eligible: false, reason: "the event states it is not mutually exclusive" }
  }

  const template = String((event && event.template) || "").toLowerCase()
  if (stated !== true && template !== ONE_WINNER_TEMPLATE) {
    return {
      value: null,
      eligible: false,
  ...
  for (const contract of contracts) {
    const { ask, atTickFloor } = readQuote(contract)
    if (ask === null) {
      return { value: null, eligible: false, reason: "not every outcome is offered" }
    }
    sum += ask
    if (atTickFloor) { tickFloorSum += ask; tickFloorLegs += 1 }
  }
  ...
  return {
    ...
    eligible: true,
    reason: null,
    legs: contracts.length,
    askSum: round(sum, 4),
    // How much of that sum is legs resting on the minimum tick. On a wide field
    // this is most of it, and it is the difference between "the venue charges a
    // wide margin" and "ninety longshots cannot be quoted below a cent".
    tickFloorLegs,
    tickFloorSum: round(tickFloorSum, 4),
  }
}
...
function summarizeEvent(event, now) {
  ...
  return {
    ...
    // The decomposition, so a wide margin can be read apart from a wide field.
    overroundAskSum: overround.askSum === undefined ? null : overround.askSum,
    tickFloorLegs: overround.tickFloorLegs || 0,
    tickFloorSum: overround.tickFloorSum || 0,
...
function finishRollup(r) {
  const { _spreadSum, _overroundSum, _askSum, _tickFloorSum, ...rest } = r
  return {
    ...rest,
    ...
    // What share of the summed asks behind that margin is legs resting on the
    // minimum tick. High means the margin is mostly granularity, not spread.
    overroundTickFloorShare: _askSum > 0 ? round(_tickFloorSum / _askSum, 4) : null,
...
function accumulate(target, summary, spreadSum) {
  ...
  if (summary.overroundPlausible) {
    ...
    target._askSum += summary.overroundAskSum || 0
    target._tickFloorSum += summary.tickFloorSum || 0
    target.tickFloorLegs += summary.tickFloorLegs || 0
...
function summarize(rawEvents, { rowLimit = DEFAULT_ROW_LIMIT, now = Date.now() } = {}) {
  ...
  return {
    ...
    totals: {
      ...
      overroundTickFloorShare: finished.overroundTickFloorShare,
      tickFloorLegs: finished.tickFloorLegs,
  ...
  const { value, fromCache } = await cached(key, SNAPSHOT_CACHE_MS, async () => {
    ...
    return {
      ...
      notes: {
        ...
        overround: `Overround and dutch books cover single-winner (template: categorical) events only; multi-winner and single-contract events are excluded, as are events with any unquoted leg and events whose overround falls outside ±100% (see overroundExcluded for the breakdown). avgOverround is only a platform figure when overroundRepresentative is true, meaning at least ${OVERROUND_MIN_SAMPLE} eligible events; below that read the per-event values, not the average. Note also that on a large field the sum of asks is inflated by longshots resting at the minimum tick, so a big margin there is a granularity artefact — overroundMaxLegs gives the largest field in the sample, and overroundTickFloorShare gives the share of the summed asks that is legs resting on the minimum priceIncrement — a high share means the margin is granularity rather than spread`,
