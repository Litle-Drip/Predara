...
async function fetchEventBySlug(slug, venue = "com", deps = {}) {
  ...
  for (const host of hosts) {
    ...
    if (Array.isArray(parsed) && parsed.length === 0) {
      lastFailure = { status: 404, error: notFoundMessage(slug, venue) }
      continue
    }
...
function notFoundMessage(slug, venue) {
  return venue === "us"
    ? `No event with slug "${slug}" on polymarket.us or polymarket.com. ` +
      "polymarket.us is a separate US-regulated exchange — its listings are not always mirrored on polymarket.com."
    : `No Polymarket event with slug "${slug}"`
}
