...
async function relay(targets, message) {
  ...
  const results = await Promise.all(targets.map(async (t) => {
    try {
      ...
      // `t` itself can be the malformed thing, and dereferencing it in the
      // handler turned one bad target into a thrown 502 for the whole relay —
      // silencing the working targets this is written to keep independent.
      return { kind: t && t.kind, ok: false, error: err.message }
    }
  }))
